...
This repository contains Python scripts to build the "Agency OS" Notion template (HeyIsmail version).

## Files

*   `build_agency_os.py`: The main script that builds the Agency OS structure (Dashboard + Databases).
*   `notion_api.py`: A custom helper class for Notion API interactions.
...
2.  **Environment Variables**:
    ...
    **Ensure the Parent Page is not archived.**
...
3.  Create interconnected sub-databases:
    *   **Team** (Inline on Dashboard, with populated dummy data)
    *   **Projects, Tasks, Clients, Finance, Meetings, SOPs** (Subpages)
4.  Generate a dashboard with "Quick Links" columns and a Team section.
